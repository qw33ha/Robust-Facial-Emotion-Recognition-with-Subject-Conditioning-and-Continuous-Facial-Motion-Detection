"""
evaluation/eval_ravdess_cnn_lstm.py

Purpose:
    Offline batch evaluation for the RAVDESS-trained CNN-LSTM
    checkpoint. Loads weights/best_ravdess_cnn_lstm.pth, groups frames
    by video, runs one clip-level inference per video on the
    actor-independent held-out split (actors 21-24), and reports
    overall/per-class metrics plus the confusion matrix.

Pipeline:
    - Load weights/best_ravdess_cnn_lstm.pth into CNNLSTM128.
    - Walk ./output_gray_frames/<emotion>/ and group frames by video
      prefix "NN-NN-...-NN_actor_YY_<emotion>", keeping only videos
      whose actor_id is in VAL_ACTOR_IDS = {21, 22, 23, 24}, matching
      the val split in training/train_ravdess_cnn_lstm.py.
    ...
    - Save the confusion matrix to
      outputs/ravdess_cnn_lstm_confusion_matrix.png.

Input:
    weights/best_ravdess_cnn_lstm.pth   Checkpoint produced by
                                        training/train_ravdess_cnn_lstm.py.
    ./output_gray_frames/<emotion>/     Frames grouped by emotion.

Output:
    Printed metrics on stdout.
    outputs/ravdess_cnn_lstm_confusion_matrix.png    Saved confusion matrix.
...
Notes:
    - The per-epoch val confusion from training is saved separately
      to outputs/ravdess_cnn_lstm_val_confusion.png; this eval script
      is the canonical standalone reproduction.
...
CONFUSION_MATRIX_PATH = str(OUTPUTS_DIR / "ravdess_cnn_lstm_confusion_matrix.png")

# Actor-independent held-out split. Must match the val_actor_ids used in
# training/train_ravdess_cnn_lstm.py (actors 21-24).
...
# in training/train_ravdess_cnn_lstm.py so label indices align.
CLASS_NAMES = [
    "angry",
    "disgust",
    "fearful",
...
# =========================================================
# 2. Model (mirrors training/train_ravdess_cnn_lstm.py)
# =========================================================
...
def save_confusion_matrix_png(confusion, class_names, output_path,
                              title="RAVDESS CNN-LSTM Confusion Matrix"):
...
def main():
    ...
    save_confusion_matrix_png(
        ...
        title="RAVDESS CNN-LSTM Confusion Matrix — Held-out Actors 21-24",
    )
