"""
evaluation/eval_ravdess_cnn_lstm_residue.py

Purpose:
    Offline batch evaluation for the RESIDUE CNN-LSTM checkpoint
    produced by training/train_ravdess_cnn_lstm_residue.py. Loads
    weights/best_ravdess_cnn_lstm_residue.pth, rebuilds the per-actor
    neutral template (using the same warm-start encoder checkpoint
    that the training script used), and runs one clip-level inference
    per video on the actor-independent held-out split (actors 21-24).

    Reports overall / per-class accuracy, prediction distribution, and
    the 7x7 confusion matrix (counts + row-normalized percentages),
    saving the confusion matrix as a PNG.

Pipeline:
    - Load weights/best_ravdess_cnn_lstm_residue.pth into
      CNNLSTMResidue128. The checkpoint already contains the frozen
      encoder weights, so no separate warm-start load is required.
    - Walk ./output_gray_frames/neutral/ and build a deterministic
      per-actor neutral template: for every actor, encode every
      neutral frame through the checkpoint encoder and take the mean.
      This matches the val-time template used by
      train_ravdess_cnn_lstm_residue.py (mean of all neutral frames).
    - Walk ./output_gray_frames/ and group frames by video prefix,
      keeping only videos whose actor_id is in VAL_ACTOR_IDS =
      {21, 22, 23, 24}.
    - For each video, uniformly sample SEQ_LEN frames via
      torch.linspace, build a [T, 1, 128, 128] clip, subtract the
      actor's neutral template (inside model.forward), and run one
      forward pass.
    - Save the confusion matrix to
      outputs/ravdess_cnn_lstm_residue_confusion_matrix.png.

Input:
    weights/best_ravdess_cnn_lstm_residue.pth   Checkpoint.
    ./output_gray_frames/<emotion>/             Frames grouped by emotion
                                                (needs neutral/ too for
                                                the template).
...
Notes:
    ...
    - The neutral template is deterministic at eval time (mean over
      all neutral frames of the actor), not the K=8 random sampler
      that the training loop uses — this matches the val path of the
      training script.
"""
...
MODEL_PATH = str(PROJECT_ROOT / "weights" / "best_ravdess_cnn_lstm_residue.pth")
TEST_DIR = str(PROJECT_ROOT / "output_gray_frames")
CONFUSION_MATRIX_PATH = str(OUTPUTS_DIR / "ravdess_cnn_lstm_residue_confusion_matrix.png")

# Must match train_ravdess_cnn_lstm_residue.py.
...
# =========================================================
# 2. Model (mirrors training/train_ravdess_cnn_lstm_residue.py)
# =========================================================
...
def build_actor_neutral_means(encoder, data_dir, actor_ids, batch_size: int = 64):
    """
    For each actor in `actor_ids`, encode every neutral frame in
    ./output_gray_frames/neutral/ through the given encoder and return
    the mean feature. Matches the val-time template of
    ...
    if missing:
        raise RuntimeError(
            ...
            "The residue model needs at least one neutral frame per actor to "
            "subject-normalize them."
        )
...
def collect_video_samples(test_dir: str, actor_ids):
    """
    Returns list of (sorted_frame_paths, class_idx, actor_id) for every
    RAVDESS video whose actor_id is in `actor_ids`.
    """
...
def save_confusion_matrix_png(confusion, class_names, output_path,
                              title="RAVDESS CNN-LSTM (residue) Confusion Matrix"):
...
def main():
    ...
    save_confusion_matrix_png(
        ...
        title="RAVDESS CNN-LSTM (residue) — Held-out Actors 21-24",
    )
