...
Purpose:
    End-to-end baseline for the CNN-LSTM on RAVDESS grayscale frame
    clips. Unlike training/train_ravdess_cnn_lstm.py (which warm-starts
    the VGG-style encoder from a frame-level checkpoint and freezes it),
    this script trains the encoder from scratch together with the
    temporal head — i.e. no warm-start, no freezing.

    This variant exists as the apples-to-apples comparison for the
    warm-started + frozen version: same data split, same architecture,
    same optimiser recipe; the only difference is that every parameter
    (encoder + LSTM + attention pool + MLP head) is trained jointly.
...
Pipeline:
    ...
    - Encoder: VGGStyleCNN128 features (+ adaptive pool + projection to
      feat_dim). Randomly initialised, fully trainable (including BN
      running stats).
...
Output:
    weights/best_ravdess_cnn_lstm_e2e.pth               Best checkpoint.
    outputs/ravdess_cnn_lstm_e2e_val_confusion.png      Best-epoch confusion.
...
Notes:
    ...
    - Class labels follow alphabetical sorted() order so inference-time
      ...
      and the warm-started CNN-LSTM.
    - Expected behaviour: comparable or lower val accuracy than the
      warm-started + frozen variant, because the ~5M encoder parameters
      have to be learned from 1440 videos (actor-independent train set)
      and are much easier to overfit. This script is meant to quantify
      how much the two-stage (frame pretrain -> freeze + LSTM) recipe
      actually buys, not to be the deployed model.
"""
...
class VGGStyleCNN128Encoder(nn.Module):
    ...
    linear projection). Identical structure to the warm-started variant
...
def save_confusion_matrix_png(
    ...
    class_names,
    output_path,
    title="RAVDESS CNN-LSTM (end-to-end) Val Confusion Matrix",
):
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    # End-to-end: every submodule trains normally. No encoder pinning.
    model.train()
...
def main():
    ...
    best_model_path = str(WEIGHTS_DIR / "best_ravdess_cnn_lstm_e2e.pth")
    best_confusion_path = str(OUTPUTS_DIR / "ravdess_cnn_lstm_e2e_val_confusion.png")
    ...
    # End-to-end: no warm-start, no freezing. Every parameter trains.
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    n_total = sum(p.numel() for p in model.parameters())
    print(f"End-to-end training (no warm-start, no freeze).")
